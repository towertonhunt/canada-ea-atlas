#!/usr/bin/env python3
"""Merge classified federal condition shards into federal_conditions_v2.json.gz.

Joins shard classifications (data/conditions/shards_federal/out/*.json) back
onto the extracted records (federal_conditions_extracted.json.gz), drops
discards, validates enums, and dedupes the annual-report inflation: identical
(project_id, normalized measure_text) pairs collapse to their first
occurrence (the Milton Logistics Hub corpus doc repeats decision-statement
conditions inside compliance-report tables).

Usage: python3 scripts/merge_federal_conditions.py [--allow-partial]
...
SHARD_IN = os.path.join(ROOT, 'data', 'conditions', 'shards_federal', 'in')
SHARD_OUT = os.path.join(ROOT, 'data', 'conditions', 'shards_federal', 'out')
OUT = os.path.join(ROOT, 'data', 'conditions', 'federal_conditions_v2.json.gz')
